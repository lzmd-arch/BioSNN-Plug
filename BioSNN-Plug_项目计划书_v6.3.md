...
> 此前从没被 CI 碰过。
> ⑰ **§九「在线学习」那一格的指标名要待补**：表里写的是「e-prop 序列任务 PPL」，但 `PPL`
> 定义在「对离散词表的下一步分布」上，而全仓顺序任务上量的都是**分类准确率**（sMNIST 多随机种子
> 口径 0.7764；与 BPTT 的量化差距报告同样是准确率口径，0.5274 vs 0.7602）——**没有任何一处算过
> 困惑度**，因此这一格在现有任务上**没有定义**。本版只在 §九 表下补记这个缺口并列出两条出路
...
| :--- | :--- | :--- | :--- | :--- |
| 在线学习 | e-prop 序列任务 PPL ⚠️ | 可用；**产出与代理梯度基线的量化差距报告**（口径见本表下） | 差距收窄或有明确归因分析 | --- |
| 持续学习 | 平均增量准确率 | 65%+ | 85%+ | --- |
| 抗遗忘 | 平均遗忘（AF） | < 8% | < 5% | --- |
| 抗遗忘 | 反向迁移（BWT） | > -10% | > -5% | --- |
| **调用决策** | **触发准确性** | --- | --- | **70%+** |
...
⚠️ **表里「e-prop 序列任务 PPL」这一格的指标名待补**（2026-09-25 补记，只登记缺口，不在本阶段实现）。
`PPL`（困惑度）定义在「对离散词表的下一步分布」上，而本项目顺序任务上量的一律是**分类准确率**：
sMNIST 多随机种子口径 **0.7764**，e-prop vs. BPTT 的量化差距报告同样是准确率口径
（**0.5274 vs 0.7602**，差 **+0.2328**，出自 `sweep_results/w2_bptt.log`）。**全仓没有任何一处
计算过困惑度**，所以这一格在现有任务上**没有定义**、目前无法验收。两条出路留到第二阶段开工时裁决：
① 把指标换成实际在量的口径（顺序任务准确率），量化差距报告即它的量化形式；② 保留 PPL，第二阶段先添
一个序列**预测**任务（例如预测下一帧 / 下一输入）让困惑度有定义。第一阶段的现状**不欠这个指标**：
§七 的开阶段成功标准只要求「e-prop RSNN 顺序任务可用」，§九 这一格本来就是**第二阶段**的目标。
