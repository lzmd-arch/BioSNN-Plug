...
[中文](README.md) · [English](README.en.md) · [プロジェクト計画書](BioSNN-Plug_项目计划书_v6.2.md) · [プラグイン開発ガイド](docs/plugin_guide.ja.md) · [アーキテクチャ決定記録](docs/adr/README.ja.md)
...
| 研究コード：三つの検証ライン | **第 1 フェーズの実装完了**——W1 Hebbian 知覚 MNIST **98.01%** ✓；W2 e-prop 系列 sMNIST **77.48%**（活動ニューロン 1.0000）✓；W3 R-STDP CartPole **基準未達**（≥ 200 歩）。数値と再現記録は[再現記録](docs/reproducibility.ja.md) 参照 |
...
データは下から上へ流れる。✅ は現在のリポジトリで既に動くもの、⬜ はプロジェクト計画書にあり未実装のもの——同じ図に描いているのは、この図が同時にロードマップでもあるためだ。
...
flowchart TB
    ...
    subgraph L4["実行層"]
        ACT["行動生成 + モダリティデコーダ<br/>R-STDP + 報酬予測 Critic<br/>⬜ 第 1〜3 フェーズ"]
    end

    subgraph L3["認知層（認知コア）"]
        WM["ワーキングメモリ<br/>RSNN + ALIF + e-prop<br/>⬜ 第 1 フェーズ"]
...
コードは [Apache-2.0](LICENSE)、ドキュメントとプロジェクト計画書は [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/)。データセットは各データソースのライセンスに従い、本リポジトリはデータミラーを提供しない；ダウンロードと前処理スクリプトは第 1 フェーズとともに提供する。
