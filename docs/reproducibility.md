...
| W2 e-prop 认知层 | [`research/eprop/README.md`](../research/eprop/README.md) | sMNIST 77.48%；活跃神经元比例 1.0000（阈值 > 60%）。**旁证 SHD 是负面结果**：测试准确率 0.0534 ≈ 随机（0.05）——同一套为 sMNIST 选的超参在 SHD 上不学，诊断与后续做法见 README。⚠️ **77.48% 是 30 epoch 预算下的结果，不是收敛值**：同一批种子把预算放到 100 epoch 后测试准确率 85.19%（5 种子均值；配对差均值 **+7.55** 个百分点），**到第 100 轮仍未收敛**——见 README 的「两个预算」一节 |
...
上表的每个数字都有**原始产物**可查，它们在 [`sweep_results/`](../sweep_results/) 里，且**随仓库入库**
（2026-09-23 起——此前它被 `.gitignore` 忽略，也就是说那些数据只存在于一块硬盘上，那不叫"保留"）。
目录清单、每个 json 的字段含义、以及「哪个目录支撑 README 里的哪个数字」，见
...
**报"验收数字"只用全新种子那批**（W3 是 `step14_confirm`，种子 45–64；用于调参的那批是 25–44，
两批都过线，但两者之间约 27% 的差距本身就是结论的一部分）。
