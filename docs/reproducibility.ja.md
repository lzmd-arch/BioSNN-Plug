...
**第 0 フェーズの三つの成功基準（計画書 §七）は、それぞれ確認済みです**：

| 基準 | 状態 |
| :--- | :--- |
| スケルトンライブラリが `pip install` できる | ✅ 2026-09-23 に `biosnn-bus 0.1.0` を PyPI へ公開（trusted publishing / OIDC、手順は [`release.yml`](../.github/workflows/release.yml)） |
| かつ CI を通る | ✅ 12 ジョブすべて緑（クリーンな venv に wheel を入れてテストを走らせる脚も含む） |
| デモが GPU の無いまっさらな環境で動く | ✅ **numpy と matplotlib だけ**の環境で [`examples/quickstart_register_plugin.py`](../examples/quickstart_register_plugin.py) を実行：パッケージの不在を検出して PyPI から自動インストールし、デモ全体が完走（復号誤差 0.0161、レベル符号化の量子化ステップ 1/32 と一致）。中・英・日の 3 版すべて実行 |

最初の二つは CI とリリースパイプラインが継続的に見ています。三つ目は **Colab を模擬して手動で
確認**しました——このマシンに Colab は無いため、確認したのは「まっさらな環境 + PyPI から導入 +
完走」という経路であり、「Colab のタブで一度クリックした」ことではありません。
...
| W2 e-prop 認知層 | [`research/eprop/README.ja.md`](../research/eprop/README.ja.md) | sMNIST 77.48%；活動ニューロン比率 1.0000（閾値 > 60%）。**傍証 SHD は負の結果**：テスト精度 0.0534 ≈ 偶然水準（0.05）——sMNIST 用のハイパーパラメータでは SHD を学習しません。診断と今後の進め方は README 参照。⚠️ **77.48% は 30 エポック予算での結果であり、収束値ではありません**：同じシードで予算を 100 エポックに開放するとテスト精度 85.19%（5 シード平均；対比較の差は平均 **+7.55** パーセントポイント）に達し、**第 100 エポックでも依然として収束していません**——README の「二つの予算」を参照 |
...
上の表の各数値には**生の成果物**が対応しており、それらは [`sweep_results/`](../sweep_results/) にあり、
**本リポジトリで追跡されるようになりました**（2026-09-23 以降——それ以前は `.gitignore` に入っており、
データは一枚のディスクにしか存在しませんでした。それは「保存」とは言いません）。ディレクトリ一覧、
json の各フィールドの意味、そして「どのディレクトリが README のどの数値を支えているか」は
[`sweep_results/README.md`](../sweep_results/README.md) にあります。

保存方針は三つ：**削除しない・上書きしない**——新しい実験は新しいディレクトリを作り、結論が後に
否定された古いディレクトリもその旨を注記してそのまま残します（例：W1 の、実行中の編集で汚染された回）。
1 回の実行 = 1 つの `.json`、1 回の掃引 = 1 つのディレクトリ（集計 `.log` は**ディレクトリと同名とは限りません**——調整期のバッチログは複数の分岐を一度に走らせ、結果は複数のディレクトリに分かれます。`sweep_results/README.md` の「バッチログ」の節を参照）。そして
**データセットはこれに含まれません**——`data/` は依然として追跡せず、§12.1 に従って
`scripts/download_data.py`（ファイルごとの検証）で再現します。

論文を書くとき：曲線は json の `curve` から、中央値と最小値は集計ログか自身の集計から、
そして**受入数値は新品シードの批次からのみ**報告してください（W3 では `step14_confirm`、シード 45–64。
調整用の批次は 25–44 で、両方とも合格ですが、両者の約 27% の差自体が結論の一部です）。
