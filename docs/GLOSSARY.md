...
| :--- | :--- | :--- | ---: | :--- |
| 计划书 | project plan | プロジェクト計画書 | 104 | 正式版是 `BioSNN-Plug_项目计划书_v6.2.md`；`v6.3` 是并入第一批引用考证更正的修订版，两版并列保留（关系见根 README 开头）。**不译作 "design doc" 或 "spec"** |
...
| 读出 | readout | 読み出し | — | 接在隐藏层之后的线性分类器。**它怎么训练各条线不同，不要说成一种**：W1 现行默认是岭回归**闭式解**（累积 `XᵀX`/`XᵀY` 后一次解出，无反向传播、无迭代），交叉熵 + SGD 只作对照保留；W2 用交叉熵 + 梯度下降；W3 用 TD 误差调制的 Actor-Critic 信号。**三者都是监督的**，与隐藏层那条局部规则不是一回事 |
