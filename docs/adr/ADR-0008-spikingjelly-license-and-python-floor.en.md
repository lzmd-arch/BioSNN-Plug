...
- **Later correction (2026-09-23, after actually inspecting the external repository)**: two sentences in decision 3 do not match the facts of `ChFrenkel/eprop-PyTorch`. Corrected here rather than in the body, per repository convention:
  1. **It is not "pinned to a torch API that 2.14 cannot satisfy"** -- the repository has **no dependency manifest or version constraint at all** (no `requirements.txt`, no `pyproject.toml`, no CI). The mechanical reason it no longer runs is two uses of `dtype=np.int` in `setup.py`; that alias was removed in **NumPy 1.24**.
  2. **It cannot serve as the reference for ALIF eligibility traces** -- the repository has **explicitly removed ALIF** (`main.py`: "Support for the ALIF neuron model has been removed."; `models.py`: `assert self.model == "LIF"`), and it contains a single initial commit from 2022-02-18, so no ALIF version exists in its history either. ALIF must be checked against the **official** implementation [15] `IGITUGraz/eligibility_propagation` (e.g. `Figure_3_and_S7_e-prop_tutorials/tutorial_evidence_accumulation_with_alif.py`) and against the original Bellec et al. 2020.
  **What it can still provide is therefore narrower**: the LIF + evidence-accumulation chain (hard-coded Eqs. (4)/(25), the `F.conv1d` causal-convolution vectorisation of eligibility traces, and the trick of lengthening the leak time constant to cover second-scale dependencies). The licence conclusion is unchanged -- Apache-2.0, compliant, safe to depend on; but it is **not on PyPI and has no tags**, so it can only be anchored at commit `0f32a8f2`, and in practice remains reading-only.
