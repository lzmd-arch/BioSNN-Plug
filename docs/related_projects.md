...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **BioSNN-Plug**（本项目） | 研究原型 | 纯局部学习的 SNN 认知核心 + 模态插件；SNN 决定何时调用 LLM | e-prop（ALIF）+ R-STDP + TD-LTP Critic + 核化 IB-Hebbian；**无代理梯度** | numpy 骨架库 + PyTorch/SpikingJelly 研究线 | 本仓库 Apache-2.0；骨架库 `biosnn-bus` 独立 semver | 第一阶段三条线已验收（W1 97.79% / W2 77.48%——30 epoch 预算，放到 100 epoch 再涨 **+7.55** 个百分点且仍未收敛 / W3 中位数 237.8 步），数字见[复现记录](reproducibility.md) | —— |
