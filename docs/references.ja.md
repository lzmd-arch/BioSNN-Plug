...
| 2 | Pes, L., Yin, B., Stuijk, S., & Corradi, F. (2025). Traces Propagation: Memory-Efficient and Scalable Forward-Only Learning in Spiking Neural Networks. *arXiv preprint* arXiv:2509.13053. | https://arxiv.org/abs/2509.13053 | metadata-error | ⚠️ **データセット名の誤り**：プロジェクト計画書の本文は「MNIST と SHD 上で」と書いていますが、原典は **N-MNIST**（イベントカメラのデータセット）であり、この二つは同じものではありません。原典の全文で MNIST は一度も使われていません。⚠️ **出典が不完全**：この論文はすでに *Neuromorphic Computing and Engineering* **6(1):014002 (2026)**、DOI 10.1088/2634-4386/ae2ef9 として発表されています。ジャーナル版を補うことを勧めます。また、「シナプスごとに保存するため O(N²) になる」という点は資格痕跡系の規則については成り立ちますが、原典がその計算量を明示的に帰属させているのは OSTTP/OSTL であり、e-prop の空間計算量を単独で示しているわけではありません——プロジェクト計画書がそれを e-prop だけに帰属させるのは原典を超えた拡張にあたります。 |
