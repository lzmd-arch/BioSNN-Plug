...
本リポジトリでは、別の種類の誤りも起きています——結論には触れないのに**内容が消える**誤りです：
参考文献 [4] の行の列構造の説明に、エスケープされていない `|` を三つ使っていました。
GFM では表の `|` は**バッククォートの内側でも区切り文字のまま**であり、`\|` と書く必要が
あります。余分なセルはレンダリング時に**そのまま破棄**されます——この行では、三つ目の
縦線から行末までの 877 文字のソース（Table 4/5 の MNIST の数値と、逐字の HSIC 引用
二つ）がまるごと表示されません——備考セルの可視テキストは 1,089 文字から 260 文字に
落ちます。一方、生のファイルを読む限りは正常に見えます。
2026-09-25 に三言語そろって修正しました。教訓は：**この表の確認は生のファイルを読むだけでは
済みません**——レンダリング結果を見るか、少なくとも各行のセル数がヘッダーと
一致するかを数えることです。
...
上の表で `verified` と付いた「逐字確認」はすべて、ローカルに保管した全文コピーに基づいています。
これらのコピーは**リポジトリに入れません**——第三者の論文全文の再配布はそれぞれのライセンスに
制約されます（[3] は J Neurosci の all-rights-reserved、[19] は `cc_no`）。§12.1 の
「データミラーは提供せず、入手方法のみ提供する」と同じ原則に従い、ここには**出典と指紋**だけを登録します：

| 引用 | 出典 | ローカルコピー | 抽出方法 |
| :--- | :--- | :--- | :--- |
| [1] Bellec et al. 2020 | `nature.com/articles/s41467-020-17236-y` | 97,228 B · sha256 `6a02005749ee0949…` | 出版社 HTML をタグ除去 |
| [3] Frémaux et al. 2010 | `jneurosci.org/content/30/40/13326` | 90,510 B · sha256 `86e2fe258282f113…` | 同上（PMC6634722 と相互照合） |
| [4] Pogodin & Latham 2020 | `arxiv.org/abs/2006.07123` | 125,749 B · sha256 `c860d802e48557a0…` | arXiv LaTeXML HTML を**セル単位で解析** |
| [19] Noè et al. 2026 | `biorxiv.org/content/10.64898/2026.02.12.705507v1.full` | 52,690 B · sha256 `5255b7fda6ed6e5e…` | 出版社 HTML をタグ除去（初回取得は Cloudflare `error code: 1015` で遮断、2026-09-25 に再取得） |

3 点を明記しておかないと、これらの指紋は実際より強いものとして読まれてしまいます：

1. **PDF のテキストストリームではありません。** [4] の Table 3/4/5 を `pdftotext -layout` で読むと
   行がずれます（本リポジトリはこれで一度失敗しています）ので、表は HTML からセル単位で解析しています。
2. **ローカルコピーは、上の表の数値を生んだ抽出物と同一とは限りません。** 例：[1] の備考には
   「本文をタグ除去して 146,503 文字」とありますが、ローカルのこの `.txt` は 97,002 文字です——
   同じ成果物ではありません（おそらく一方は本文のみ、他方は参考文献を含む）。したがって
   **ハッシュが識別するのはこのローカルファイルであり、上の結論の唯一の根拠ではありません**。
   根拠は備考にある**逐字引用そのもの**で、コピーがなくても公開されている出典に当たって確認できます。
3. **「逐字」自体も細部では逐字でないことがあります。** [19] の著者の自己定位の一文は、原文では
   `Predictive Eprop`——**ハイフンがありません**。ハイフン付きの `Predictive E-prop` はタイトルと
   本文の残り 42 箇所での書き方です。台帳の初版はこの一箇所をハイフン付きで引用しており、2026-09-25 に
   コピーを取り直して逐字比較して初めて分かりました。同じ照合で、コピーで確認できる他の英文逐字引用
   （全 8 件）はすべて一致しました。引用中のハイフン・上下付き文字・ギリシャ文字は
   原文どおりに写し、習慣で正規化しないこと——そうでなければ「逐字」という語が成立しなくなります。
...
| §三、§八 の選定表、§十一 のリスク表、§3.1 の重要修正 1、**§6.2、§十、§12.4、§十三** | Trace Propagation を「e-prop のストレージ最適化：O(N²) → O(N)」と書いています | [ADR-0010](adr/ADR-0010-eprop-quadratic-storage-and-trace-propagation.ja.md) に従って書き換えます：TP は**別の規則**（痕跡はシナプス単位ではなくニューロン単位）であり、e-prop の省メモリ版ではありません。両者は代替ではなく並列です。あわせて §3.1 がこれを本条の补救措置とする記述を削除します。**2026-09-25 追記**：本行は当初、前半の 4 か所しか挙げていなかったため、v6.3 の最初の訂正では **§6.2 の箇条書きとメモリ制約の宣言、§十 のリスク表 2 行、§12.4 の ADR の例、§十三 の革新性の行**が**漏れていました**——この 5 か所は依然として「メモリ節約のため TP を採用せよ」と読者に伝えていました。現在はすべて修正済みで、**§6.2 のメモリ制約の宣言は再計算しました**（元の 2 つの数値はバッチ次元の分を掛け忘れていました。導出は v6.3 §6.2 を参照）。教訓：**帰属の誤り**を訂正するときは、その時点で列挙した箇所のリストを順に潰すのではなく、同じ主張が他にどこに出てくるかを全文検索すること |
