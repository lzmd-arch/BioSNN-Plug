...
写这份探针的时候 W3 还没通过验收（见 [`README.md`](README.md)），现在通过了——但那句话
与探针的用处无关：它判定的是「这个编码上**存在**能解 CartPole 的线性策略吗」，而答案是
**存在**，正是这条答案后来把「容量不足」从嫌疑名单上划掉。

一个待查的假设是：**瓶颈在设计容量，不在学习规则**——Actor 是一个线性策略，作用在固定的
随机群体编码上。R-STDP 解 CartPole 的常见做法是用大得多的特征集，或让 STDP 参与塑造特征
本身。
