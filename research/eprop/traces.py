...
def eprop_gradient(
    ...
    **O(N²)** 量级（随连接数）。本函数解决的是"能不能在 8GB 上跑完"（§6.2 的硬约束）。

    §3.1「关键修正 1」把 Trace Propagation（Pes et al. 2025）写成本条的**补救措施**。
    逐条查过原文之后**不采纳**：它不是 e-prop 的省内存版本，而是换掉空间信用分配的另一条
    规则（论文把两者分列两行、分轴评价），而且它覆盖的神经元模型是 LIF，在本条目的验收配置
    （ALIF，β=0.07）上没有推导。裁决与三条证据见 `docs/adr/ADR-0010`；
    论文那条规则本身的最小复现在 `examples/paper_pes2025.py`。

    **本条真正该做的那一步已经做了**：``epsilon_v`` 的递推不含后突触下标，已从
    ``(batch, n_pre, n_post)`` 收到 ``(batch, n_pre, 1)``——逐位等价，验收形状下墙钟 −18%。
    """
    steps, batch, n_post = v_scaled.shape
    if z_post.shape != v_scaled.shape:
        raise ValueError(
