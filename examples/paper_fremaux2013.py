...
#
# **本脚本不演示"有偏信号导致学不会"那条断言**——那是另一篇论文（Frémaux et al. 2010），
# 现在由 [`examples/paper_fremaux2010.py`](paper_fremaux2010.py) 负责：它用固定温度的
# Boltzmann 策略把动作分布保持在混合状态，比较"全局滑动平均"与"按刺激的预测"两种基线。
# 那个脚本复现了论文那条结论的**结构**，但**没有**复现出 25%σR 这个具体阈值——
# 见 `research/rstdp/README.md` 的已知边界第 11 条。
#
